'use client' // Necessário para usar hooks e estados

import axios from 'axios';
import { useEffect, useState } from 'react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';
import Alert, { AlertProps } from '@/app/components/alert_modal';

export default function LoginPage() {
  const [email, setEmail] = useState<string>("");
  const [senha, setSenha] = useState<string>("");
  const [mostrarSenha, setMostrarSenha] = useState<boolean>(false);
  const [lembrarDeMim, setLembrarDeMim] = useState<boolean>(false);
  ...
  const fazerLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        setInfo({type:'success', message:'Login realizado com sucesso!'})
        router.push("/pages/"+res.data.url || '/')
      }
    } catch (e: any) {
      setInfo({type:'error', message:'Erro ao realizar login. Verifique suas credênciais, e tente novamente!'})
      console.log(e)
    }
  };
  useEffect(() => {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-600 flex flex-col items-center">
          Ou <Link href="/" className="font-medium text-indigo-600 hover:text-indigo-500">crie uma nova
            conta</Link>
        </p>
      </div>
          ...
          <form className="space-y-6" onSubmit={fazerLogin}>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                />
                <label htmlFor="lembrar-de-mim" className="ml-2 block text-sm text-gray-900">
                  Lembrar de mim
                </label>
              </div>
